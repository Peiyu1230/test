...
def watchAdVideo(headers):
  ...
  try:
    response = requests.post(url=url, data="type=taskCenter", headers=headers, timeout=30,verify=False).json()
    print('看广告视频')
    print(response)
    if response['status'] == 1:
      return response
